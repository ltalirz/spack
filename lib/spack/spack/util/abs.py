...
class AzureBlob:
 ...
 spack buildcache")
    ...
    def azure_container_exists(self):
        ...
        try:
            ttt = self.azure_container_client
            for blob in ttt:
                pass
        except Exception:
            return False
    ...
    def azure_blob_exists(self):
        """Returns True if the blob exists, otherwise False.
           ...
           blob property."""
        try:
            self.azure_blob_client.get_blob_properties()
        except Exception:
            return False
        return True

    def azure_get_blob(self):
        if self.azure_blob_exists():
            return self.azure_blob_client.download_blob()
        return None

    def azure_get_blob_byte_stream(self):
        return self.azure_get_blob().open(mode="rb")

    def azure_get_blob_headers(self):
        properties = self.azure_blob_client.get_blob_properties()

        headers = {
            "Content-type": properties.content_settings.content_type,
            "Content-encoding": properties.content_settings.content_encoding,
            "Content-language": properties.content_settings.content_language,
            "Content-disposition": properties.content_settings.content_disposition,
            "MD5Hash": properties.content_settings.content_md5,
        }
    ...
    def azure_list_blobs(self):
        try:
            blob_gen = self.azure_container_client.list_blobs()
            blob_list = []
    ...
    def azure_url_sas(self):
        """Generate account SAS token for Azure blob"""
