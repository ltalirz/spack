...
def _urlopen():
    s3_handler = spack.s3_handler.UrllibS3Handler()
    gcs_handler = spack.gcs_handler.GCSHandler()
    abs_handler = spack.abs_handler.ABSHandler()

    # One opener with HTTPS ssl enabled
    with_ssl = build_opener(s3_handler, gcs_handler, abs_handler, HTTPSHandler(context=ssl.create_default_context()))
...
def push_to_url(local_file_path, remote_path, keep_original=True, extra_args=None):
    ...
    if remote_url.scheme == "file":
        ...
        azureblob = abs_util.AzureBlob(remote_url)
...
def remove_url(url, recursive=False):
    ...
    if url.scheme == "s3":
        ...
        azureblob = abs_util.AzureBlob(url)
